...
class NoticeStickyNoteCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final headerColor = Color.alphaBlend(Colors.black.withValues(alpha: 0.06), meta.chipBg);
    ...
    Widget bodyContent() {
      if (notice.type == NoticeType.utility && notice.dueAmount != null) {
        return Column(
          ...
          children: [
            Text('${notice.dueAmount!.toStringAsFixed(2)} BDT',
                style: GoogleFonts.lobster(fontSize: 17, color: meta.chipFg)),
            if (notice.description.isNotEmpty) ...[
              const SizedBox(height: 6),
              Text(notice.description, style: const TextStyle(fontSize: 13, color: Color(0xFF1E1E1E))),
            ],
          ],
        );
      }
      if (notice.type == NoticeType.meal && (notice.mealLunch != null || notice.mealDinner != null)) {
        return Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            if (notice.mealLunch != null) Text('Lunch - ${notice.mealLunch}', style: const TextStyle(fontSize: 13, color: Color(0xFF1E1E1E))),
            if (notice.mealDinner != null) Text('Dinner - ${notice.mealDinner}', style: const TextStyle(fontSize: 13, color: Color(0xFF1E1E1E))),
          ],
        );
      }
      return Text(
              ...
              Container(
                ...
                child: Row(
                  children: [
                    ...
                    Expanded(
                      child: Text(meta.label, style: GoogleFonts.lobster(fontSize: 15, color: meta.chipFg)),
                    ),
                    if (notice.effectivelyPinned) Icon(Icons.push_pin, size: 15, color: priorityMeta.pinColor),
                    if (showStatusBadge || notice.status != NoticeStatus.published) ...[
                      const SizedBox(width: 6),
                      Container(
                        padding: const EdgeInsets.symmetric(horizontal: 7, vertical: 2),
                        decoration: BoxDecoration(color: Colors.white.withValues(alpha: 0.6), borderRadius: BorderRadius.circular(20)),
                        child: Text(notice.status.name,
                            style: TextStyle(fontSize: 10, fontWeight: FontWeight.w700, color: meta.chipFg)),
                      ),
                    ],
                    if (canManage)
                      PopupMenuButton<String>(
                        ...
                        icon: Icon(Icons.more_vert, size: 18, color: meta.chipFg),
                        onSelected: onManage,
                        itemBuilder: (_) => [
                ...
                child: Column(
                  ...
                  children: [
                    Text(
                      ...
                      style: GoogleFonts.lobster(fontSize: 17, color: const Color(0xFF1E1E1E)),
                    ),
                    ...
                    Text(
                      ...
                      style: const TextStyle(fontSize: 12, height: 1.4, color: Color(0xBF1E1E1E)),
                    ),
                  ],
                ),
              ),
